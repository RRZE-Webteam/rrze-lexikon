...
class Settings {
    ...
    protected function setFields() {
        ...
        if ( isset($_GET['page']) && $_GET['page'] == 'rrze-lexikon' && isset($_GET['current-tab']) && $_GET['current-tab'] == 'faqsync' ){
        ...
        foreach ($this->settingsSections as $section) {
            ...
            switch ( $this->currentTab ) {
                case 'faqsync':                    
                    ...
                    break;
                case 'doms': 
                    $btn_label = __('Add domain', 'rrze-lexikon' );
                    $get = '?doms';
                    break;
                case 'faqlog': 
                    $btn_label = __('Delete logfile', 'rrze-lexikon' );
                    $get = '?del';
                    break;
            }
    ...
    public function domainOutput(){
        ...
        if ( count($aDomains) > 0 ){
            $i = 1;
            echo '<style> .settings_page_rrze-lexikon #log .form-table th {width:0;}</style>';
            ...
            echo '</tbody></table>';
            echo '<p>' . __( 'Please note: "Delete selected domains" will DELETE every FAQ on this website that has been fetched from the selected domains.', 'rrze-lexikon' ) . '</p>'; 
            submit_button( __( 'Delete selected domains', 'rrze-lexikon' ) );
        }
    }
        ...
        foreach ( $aDomains as $shortname => $url ){
            ...
            foreach ( $this->settingsFields['faqsync'] as $field ){
                ...
                switch ( $field['name'] ){
                    ...
                    case 'categories':
                        if ( !$aCategories ){
                            $field['options'][''] = __( 'no category with source = "website" found', 'rrze-lexikon' );
                        }
    ...
    public function callbackLogfile($args) {
        if ( file_exists( $args['default'] ) ) {
            ...
            if ( $lines !== false ) {
                echo '<style> .settings_page_rrze-lexikon #faqlog .form-table th {width:0;}</style><table class="wp-list-table widefat striped"><tbody>';
                foreach ( $lines as $line ){
                    echo '<tr><td>' . $line . '</td></tr>';
                }
                echo '</tbody></table>';
            }else{
                echo __( 'Logfile is empty.', 'rrze-lexikon' );
            }
        }else{
            echo __( 'Logfile is empty.', 'rrze-lexikon' );
        }
