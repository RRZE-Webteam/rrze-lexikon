...
class Layout
{
    ...
    public function sortboxCallback($meta_id)
    {
        ...
        $output .= '<p class="description">' . __('Criterion for sorting the output of the shortcode', 'rrze-lexikon') . '</p>';
        echo $output;
    }
    ...
    public function anchorboxCallback($meta_id)
    {
        ...
        $output .= '<p class="description">' . __('Anchor field (optional) to define jump marks when displayed in accordions ', 'rrze-lexikon') . '</p>';
        echo $output;
    }
    ...
    public function langboxCallback($meta_id)
    {
        ...
        $output .= '<p class="description">' . __('Language of this FAQ', 'rrze-lexikon') . '</p>';
        echo $output;
    }
    ...
    public function fillShortcodeBox()
    {
        ...
        if ($post->ID > 0) {
            $ret .= '<h3 class="hndle">' . __('Single entries', 'rrze-lexikon') . ':</h3><p>[faq id="' . $post->ID . '"]</p>';
            $ret .= ($category ? '<h3 class="hndle">' . __('Accordion with category', 'rrze-lexikon') . ':</h3><p>[faq category="' . $category . '"]</p><p>' . __('If there is more than one category listed, use at least one of them.', 'rrze-lexikon') . '</p>' : '');
            $ret .= ($tag ? '<h3 class="hndle">' . __('Accordion with tag', 'rrze-lexikon') . ':</h3><p>[faq tag="' . $tag . '"]</p><p>' . __('If there is more than one tag listed, use at least one of them.', 'rrze-lexikon') . '</p>' : '');
            $ret .= '<h3 class="hndle">' . __('Accordion with all entries', 'rrze-lexikon') . ':</h3><p>[faq]</p>';
        }
        echo $ret;
    }
    ...
    public function changeTitleText($title)
    {
        ...
        if ($screen->post_type == 'faq') {
            $title = __('Enter question here', 'rrze-lexikon');
        }
        return $title;
    }
    ...
    public function toggleEditor()
    {
        ...
        if ($post_id) {
            if (get_post_type($post_id) == 'faq') {
                ...
                if ($source) {
                    if ($source != 'website') {
                        ...
                        add_meta_box(
                            ...
                            __('This FAQ cannot be edited because it is sychronized', 'rrze-lexikon') . '. <a href="' . $link . '" target="_blank">' . __('You can edit it at the source', 'rrze-lexikon') . '</a>',
                ...
                add_meta_box(
                    ...
                    __('Integration in pages and posts', 'rrze-lexikon'), // meta box title
        ...
        add_meta_box(
            ...
            __('Language', 'rrze-lexikon'), // meta box title
        ...
        add_meta_box(
            ...
            __('Sort', 'rrze-lexikon'), // meta box title
        ...
        add_meta_box(
            ...
            __('Anchor', 'rrze-lexikon'), // meta box title
    ...
    public function addFaqColumns($columns)
    {
        ...
        $columns['source'] = __('Source', 'rrze-lexikon');
        $columns['id'] = __('ID', 'rrze-lexikon');
        return $columns;
    }

    public function addFaqSortableColumns($columns)
    {
        $columns['taxonomy-faq_category'] = __('Category', 'rrze-lexikon');
        $columns['taxonomy-faq_tag'] = __('Tag', 'rrze-lexikon');
        $columns['lang'] = __('Language', 'rrze-lexikon');
    ...
    public function addFaqFilters($post_type)
    {
        ...
        $output .= '<option value="0">' . __('All Sources', 'rrze-lexikon') . '</option>';
