...
class Main {
    ...
    public function onLoaded() {
        ...
        add_action( 'update_option_rrze-lexikon', [$this, 'checkSync'] );
        add_filter( 'pre_update_option_rrze-lexikon',  [$this, 'switchTask'], 10, 1 );
    ...
    public function enqueueScripts() {
        wp_register_style('rrze-lexikon-style', plugins_url('assets/css/rrze-lexikon.css', plugin_basename($this->pluginFile)));
        wp_enqueue_style('rrze-lexikon-style');
    }
    ...
    public function switchTask( $options ) {
        ...
        $storedOptions = get_option( 'rrze-lexikon' );
        ...
        switch ( $tab ){
            case 'doms':
                if ( $options['doms_new_name'] && $options['doms_new_url'] ){
                    ...
                    if ( $aRet['status'] ){
                        // url is correct, rrze-lexikon at given url is in use and shortname is new
    ...
    public function setFAQCronjob() {
        ...
        $message = __( 'Next automatically synchronization:', 'rrze-lexikon' ) . ' ' . date( 'd.m.Y H:i:s', $timestamp );
